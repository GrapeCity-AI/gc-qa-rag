"""
Example: Run Forum QA Pipeline

This example demonstrates how to use the AI Knowledge Service
to run a Forum QA ETL pipeline for crawling Q&A threads:
  1. Ingestion - Fetch Q&A threads from forum API
  2. Indexing - Parse forum QA format, chunk, enrich with QA, embed, build index
...
def create_services():
    ...
    # === Processing Steps ===
    from ai_knowledge_service.implementations.steps.parsers import ForumQAParser
...
def run_ingestion(
    ...
    """Run ingestion to fetch Q&A threads from forum API."""
    from ai_knowledge_service.abstractions.models.tasks import IngestionTask

    logger.info(f"=== Starting Ingestion from Forum API: {product}/QA ===" + (f" (limit: {limit})" if limit else ""))
...
def run_indexing(services: dict, kb_id: str, kb_version_id: str, max_replies: int = 10):
    """Run indexing pipeline on ingested forum Q&A threads."""
    from ai_knowledge_service.abstractions.models.tasks import IndexingTask, StepConfig

    logger.info("=== Starting Indexing Pipeline (Forum QA Flow) ===")

    pipeline_config = [
        StepConfig(step_type="forum_qa_parser", config={
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Forum QA ETL Pipeline")
    ...
    parser.add_argument("--kb-id", help="Knowledge base ID (default: {product}-qa)")
    ...
    parser.add_argument("--max-replies", type=int, default=10, help="Maximum replies to include per thread")
    ...
    kb_id = args.kb_id or f"{args.product.lower()}-qa"
    ...
    if args.step in ["all", "index"]:
        run_indexing(services, kb_id, args.kb_version, max_replies=args.max_replies)
