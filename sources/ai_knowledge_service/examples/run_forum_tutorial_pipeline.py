"""
Example: Run Forum Tutorial Pipeline

This example demonstrates how to use the AI Knowledge Service
to run a Forum Tutorial ETL pipeline for crawling tutorial threads:
  1. Ingestion - Fetch Tutorial threads from forum API
  2. Indexing - Parse forum tutorial format, chunk, enrich with QA, embed, build index
...
Prerequisites:
  ...
  - Forum API credentials (base URLs and token)
"""
...
def create_services():
    ...
    connectors = {
        "forum_api": ForumApiConnector(),
    }

    # === Processing Steps ===
    from ai_knowledge_service.implementations.steps.parsers import ForumTutorialParser
    ...
    steps = {
        "forum_tutorial_parser": ForumTutorialParser(),
...
def run_ingestion(
    ...
    """Run ingestion to fetch Tutorial threads from forum API."""
    from ai_knowledge_service.abstractions.models.tasks import IngestionTask

    logger.info(f"=== Starting Ingestion from Forum API: {product}/Tutorial ===" + (f" (limit: {limit})" if limit else ""))
    ...
    task = IngestionTask(
        ...
        source_config={
            ...
            "connection_params": {
                "base_url_page": base_url_page,
                "base_url_thread": base_url_thread,
                "token": token,
                "product": product,
                "section": "Tutorial",
            },
...
def run_indexing(services: dict, kb_id: str, kb_version_id: str, include_header: bool = True):
    """Run indexing pipeline on ingested forum Tutorial threads."""
    from ai_knowledge_service.abstractions.models.tasks import IndexingTask, StepConfig

    logger.info("=== Starting Indexing Pipeline (Forum Tutorial Flow) ===")

    pipeline_config = [
        StepConfig(step_type="forum_tutorial_parser", config={
            "include_title_header": include_header,
        }),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run Forum Tutorial ETL Pipeline")
    parser.add_argument("--product", required=True, choices=["Forguncy", "Wyn", "SpreadJSGcExcel"],
                        help="Product name")
    parser.add_argument("--base-url-page", default=os.getenv("FORUM_API_BASE_URL_PAGE"),
                        help="Forum API base URL for page listing")
    parser.add_argument("--base-url-thread", default=os.getenv("FORUM_API_BASE_URL_THREAD"),
                        help="Forum API base URL for thread content")
    parser.add_argument("--token", default=os.getenv("FORUM_API_TOKEN"),
                        help="Forum API token")
    parser.add_argument("--kb-id", help="Knowledge base ID (default: {product}-tutorial)")
    ...
    parser.add_argument("--no-header", action="store_true",
                        help="Don't include forum name and title header in content")
    ...
    if not args.base_url_page or not args.base_url_thread or not args.token:
        logger.error("Forum API credentials required. Set via args or env vars:")
        logger.error("  --base-url-page or FORUM_API_BASE_URL_PAGE")
        logger.error("  --base-url-thread or FORUM_API_BASE_URL_THREAD")
        logger.error("  --token or FORUM_API_TOKEN")
        return 1

    # Default kb_id based on product
    kb_id = args.kb_id or f"{args.product.lower()}-tutorial"
    ...
    if args.step in ["all", "index"]:
        run_indexing(services, kb_id, args.kb_version, include_header=not args.no_header)

    if args.step in ["all", "publish"]:
        run_publishing(services, kb_id, args.kb_version)
