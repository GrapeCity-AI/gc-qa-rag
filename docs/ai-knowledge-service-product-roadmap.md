...
| 任务 | 说明 | 状态 |
|------|------|------|
| FastAPI 应用框架 | 主应用入口、依赖注入 | ✅ 完成 |
| 知识库 CRUD API | 创建、查询、更新、删除知识库 | ✅ 完成 |
| 版本管理 API | 版本创建、查询、更新、删除 | ✅ 完成 |
| 数据采集 API | 触发数据源采集任务 | ✅ 完成 |
| 索引构建 API | 触发构建任务 | ✅ 完成 |
| 发布 API | 触发发布任务 | ✅ 完成 |
| 任务管理 API | 任务列表、详情、取消、重试 | ✅ 完成 |
| 连接器 API | 连接器类型列表、测试 | ✅ 完成 |
| 环境管理 API | 目标环境 CRUD | ✅ 完成 |
| 系统 API | 健康检查、统计、配置 | ✅ 完成 |
| WebSocket | 任务进度实时推送 | ✅ 完成 |
| 错误处理中间件 | 统一错误响应格式 | ✅ 完成 |
...
# 构建与发布
POST   /versions/{id}/ingest              # 触发数据采集
...
| 任务 | 说明 | 状态 |
|------|------|------|
| 项目脚手架 | Vite + React 18 + TypeScript + Ant Design 5 | ✅ 完成 |
| API 客户端 | Axios + TanStack Query 集成 | ✅ 完成 |
| 布局组件 | Header、Sidebar、MainLayout | ✅ 完成 |
| 仪表盘页面 | 统计卡片、系统状态 | ✅ 完成 |
| 知识库列表 | 搜索、分页、卡片展示 | ✅ 完成 |
| 知识库创建向导 | 多步骤表单 (基本信息/数据源/管道/确认) | ✅ 完成 |
| 知识库详情 | Tabs 布局、版本管理、文件浏览器 | ✅ 完成 |
| 版本操作 | Ingest/Build/Publish 操作按钮和模态框 | ✅ 完成 |
| 任务列表 | 状态筛选、类型筛选、时间范围 | ✅ 完成 |
| 任务详情 | 进度展示、步骤统计、错误列表、重试 | ✅ 完成 |
| 数据源管理 | 连接器类型列表、连接测试 | ✅ 完成 |
| 发布管理 | 环境 CRUD、发布历史 | ✅ 完成 |
| 系统设置 | 配置展示 (LLM/存储/管道) | ✅ 完成 |
| WebSocket | 任务进度实时更新 | ✅ 完成 |
...
| 1.4 | 2026-01-17 | Phase 3 & 4 完成，管理 API 和 Admin UI 可用 |
